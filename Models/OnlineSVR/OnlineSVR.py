# Implementation of accurate online support vector
# regression (AOSVR) algorithm (Ma, Theiler, &
# Perkins, 2003; Martin, 2002) that follows the approach
# of Cauwenberghs and Poggio (2001) for
# incremental SVM regression.
...
class OnlineSVR:
    def __init__(self, X_train, y_train, C, eps, kernelParam, bias=0):
    ...
    def findMinVariation(self, H, beta, gamma, i):
        ...
        minValues = np.array([Lc1, Lc2, Ls[minLsIndex], Le[minLeIndex], Lr[minLrIndex]], dtype=object)
    ...
    def run(self):
        for i in range(len(self.X_train)):
            self.learn(self.X_train[i, :], self.y_train[i])

    # def get_y_predicted(self, X_test):
    #     PredictedY = []
    #     for i in range(len(X_test)):
    #         xs = X_test[i, :].flatten()
    #         res = self.predict(np.array(xs))
    #         PredictedY.append(res)
    #
    #     PredictedY = np.array(PredictedY).reshape(-1)
    #     return PredictedY

    def predict2(self, newSampleX):
        X = np.array(self.X)
        newX = np.array(newSampleX)
        weights = np.array(self.weights)
        weights.shape = (weights.size, 1)
    ...
    def get_y_predicted(self, X_test):
        PredictedY = self.predict2(X_test)
        PredictedY = np.array(PredictedY).reshape(-1)
        return PredictedY



def osvr(X, y, osvr_C, osvr_eps, osvr_kernelParam, osvr_bias, seed):
    kf = KFold(n_splits=5, random_state=seed, shuffle=True)
    scores = []
    for train_index, test_index in kf.split(X):
        X_train, X_test = X[train_index], X[test_index]
        y_train, y_test = y[train_index], y[test_index]
        osvr = OnlineSVR(X_train, y_train, osvr_C, osvr_eps, osvr_kernelParam, osvr_bias)
        osvr.run()
        y_predicted = osvr.get_y_predicted(X_test)
        acc_per_split_for_same_seed = Measures.r2_score(y_test, y_predicted)
